...
def check_pokedex_completeness():
    ...
    pokedex_fish = {fish['name'] for fish in fish_data if fish.get('show_in_pokedex', True)}
    hidden_fish = {fish['name'] for fish in fish_data if not fish.get('show_in_pokedex', True)}
    ...
    quality_percentage = (collected_qualities / total_qualities * 100) if total_qualities > 0 else 0
    
    print(f"   鱼类收集：{collected_fish}/{total_fish} ({fish_percentage:.1f}%)")
    print(f"   品质收集：{collected_qualities}/{total_qualities} ({quality_percentage:.1f}%)")
    ...
    with open(report_path, 'w', encoding='utf-8') as f:
        ...
        f.write(f"品质收集：{collected_qualities}/{total_qualities} ({quality_percentage:.1f}%)\n")
