...
class PokedexImageGenerator:
    ...
    def generate_pokedex_image(self, image_type: str = "all") -> Path:
        ...
        if image_type == "uncollected":
            ...
            fish_list = [
                f
                for f in all_fish
                if pokedex.get_fish_collected_count(f.get("name", "")) < len(QUALITIES)
            ]
            prefix = "未收集"
        else:
            # 显示所有鱼
        ...
        canvas_height = (
            legend_height
            + header_height
            + rows * (self.CARD_HEIGHT + self.CARD_MARGIN)
            + self.CARD_MARGIN
            + footer_height
        )
        ...
        for idx, fish in enumerate(fish_list):
            ...
            y = (
                ...
                + header_height
                + self.CARD_MARGIN
                + row * (self.CARD_HEIGHT + self.CARD_MARGIN)
            )
    ...
    def _draw_legend(
        self, img: Image.Image, draw: ImageDraw.Draw, legend_height: int, font
    ):
        """绘制顶部图例（鱼种和品质收集进度）"""
        ...
        # 绘制分隔线
        draw.line(
            [(0, legend_height - 1), (canvas_width, legend_height - 1)],
            fill=(210, 215, 220),
            width=2,
        )
    ...
    def _draw_header(
        ...
        for y in range(legend_height, legend_height + header_height):
            ...
            color = tuple(
                int(self.BG_COLOR[i] * (1 - ratio * 0.3) + 255 * ratio * 0.3)
                for i in range(3)
            )
        ...
        draw.line(
            [
                (0, legend_height + header_height - 1),
                (canvas_width, legend_height + header_height - 1),
            ],
            fill=(210, 215, 220),
            width=2,
        )
        ...
        draw.text(
            (title_x, legend_height + 15), title, fill=self.TITLE_COLOR, font=title_font
        )
    ...
    def _draw_card(
        self, img: Image.Image, draw: ImageDraw.Draw, fish: Dict, x: int, y: int, font
    ):
        """绘制单张卡片（优化版：圆角 + 渐变 + 阴影）"""
        # 绘制卡片阴影
        shadow_offset = 3
        ...
        shadow_img = Image.new(
            "RGBA",
            (self.CARD_WIDTH + shadow_blur * 2, self.CARD_HEIGHT + shadow_blur * 2),
            (0, 0, 0, 0),
        )
        ...
        shadow_draw.rounded_rectangle(
            shadow_rect, radius=self.CARD_RADIUS, fill=self.CARD_SHADOW_COLOR
        )
        ...
        img.paste(
            shadow_img,
            (x - shadow_blur + shadow_offset, y - shadow_blur + shadow_offset),
            shadow_img,
        )

        # 绘制卡片背景（渐变效果）
        self._draw_rounded_gradient_rect(
            draw,
        ...
        if img_path and img_path.exists():
            try:
                ...
                # 保持宽高比缩放，确保不超出背景区域
                fish_img.thumbnail(
                    (self.FISH_IMAGE_SIZE - 10, self.FISH_IMAGE_SIZE - 10),
        ...
        status = pokedex.get_collection_status(fish_name)
        dots_width = (
            len(QUALITIES) * (self.DOT_SIZE + self.DOT_MARGIN) - self.DOT_MARGIN
        )
            ...
            if is_collected:
                ...
                draw.ellipse(
                    [
                        dot_x - 2,
                        dots_y - 2,
                        dot_x + self.DOT_SIZE + 2,
                        dots_y + self.DOT_SIZE + 2,
                    ],
                    fill=(255, 255, 255),
                )
                # 主圆
    ...
    def _draw_rounded_gradient_rect(
        ...
        for y in range(y0, y1):
            ratio = (y - y0) / height
            color = tuple(
                int(color_start[i] * (1 - ratio) + color_end[i] * ratio)
                for i in range(3)
            )
    ...
    def _draw_footer(
        self, img: Image.Image, draw: ImageDraw.Draw, canvas_height: int, font
    ):
        """绘制页脚（账号和时间）"""
        ...
        draw.rectangle(
            [(0, footer_y), (canvas_width, canvas_height)], fill=(230, 233, 237)
        )

        # 绘制分隔线
        draw.line(
            [(0, footer_y), (canvas_width, footer_y)], fill=(210, 215, 220), width=2
        )
