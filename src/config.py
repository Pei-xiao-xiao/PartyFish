...
class Config(metaclass=SingletonMeta):
    def __init__(self):
        ...
        try:
            ...
            try:
                ctypes.windll.shcore.SetProcessDpiAwareness(1)  # Fallback to system aware
            except AttributeError:
        ...
        self.REGIONS = {
            ...
            "jiashi_popup": {"coords": (1237, 669, 40, 40), "anchor": "center"},  # 保留旧配置兼容
            "afk_popup":    {"coords": (1147, 678, 40, 40), "anchor": "center"},  # 保留旧配置兼容
    ...
    def __setattr__(self, name, value):
        ...
        if name in ['BASE_SCREEN_WIDTH', 'BASE_SCREEN_HEIGHT', 'screen_width', 'screen_height', 
                    ...
                    'window_offset_x', 'window_offset_y', 'game_window_titles', 'current_account']:
    ...
    def _get_default_presets(self):
        ...
        return {
            "路亚轻杆": {"cast_time": 1.8, "reel_in_time": 2.0, "release_time": 1.0, "max_pulls": 30, "cycle_interval": 0.2},
            "路亚重杆": {"cast_time": 2.5, "reel_in_time": 2.0, "release_time": 1.0, "max_pulls": 30, "cycle_interval": 0.2},
            "冰钓轻杆": {"cast_time": 0.1, "reel_in_time": 0.2, "release_time": 0.1, "max_pulls":100, "cycle_interval": 0.1},
            "冰钓重杆": {"cast_time": 0.1, "reel_in_time": 0.4, "release_time": 0.2, "max_pulls": 100, "cycle_interval": 0.1}
        }
    ...
    def _load_fish_data(self):
        ...
        if not fish_config_path.exists():
            self.startup_errors.append(f"⚠️ 关键配置文件缺失: resources/fish.json，鱼名匹配功能将不可用")
            self.fish_names_list = []
            return
        ...
        try:
            with open(fish_config_path, 'r', encoding='utf-8') as f:
                ...
                # 提取所有鱼名
                self.fish_names_list = [item['name'] for item in fish_data if 'name' in item]
        except Exception as e:
