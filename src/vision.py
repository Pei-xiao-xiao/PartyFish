...
class Vision:
    ...
    def _ensure_loaded(self):
        ...
        if self._last_scale != cfg.scale:
            print(f"[Vision] Scale changed from {self._last_scale} to {cfg.scale}. Rescaling templates...")
        ...
        try:
            ...
            if best_digit_score >= threshold:
                candidates.append({
                    ...
                    'width': sorted_scales[0] * raw_t.shape[1] # 估算最小宽度用于排序，实际去重用动态宽度
                })
    ...
    def _detect_digits_raw(self, img, threshold, return_details=False):
        ...
        for i in range(10):
            ...
            for scale in sorted_scales:
                ...
                res = cv2.matchTemplate(gray_screenshot, resized_t, cv2.TM_CCOEFF_NORMED)
    ...
    def find_template_popup(self, template_name, region=None, threshold=0.8):
        ...
        if popup_scale != 1.0:
            ...
            width = max(width, 1)
            height = max(height, 1)
            template = cv2.resize(raw_template, (width, height), interpolation=cv2.INTER_AREA)
        else:
    ...
    def draw_debug_rects(self, image, config, recognition_results=None):
        ...
        for name, rect_data in config.items():
            ...
            if name == 'bait_count':
                ...
                # 左切片（红色）：用于识别十位
                draw.rectangle([x, y, x + slice_width, y + h], outline=(255, 0, 0), width=1)
                
                # 右切片（蓝色）：用于识别个位
                draw.rectangle([x + w - slice_width, y, x + w, y + h], outline=(0, 0, 255), width=1)
                
                # 中切片（黄色）：用于识别一位数
                center_start = x + (w - slice_width) // 2
                draw.rectangle([center_start, y, center_start + slice_width, y + h], outline=(255, 255, 0), width=1)
