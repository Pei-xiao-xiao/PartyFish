...
class ProfitInterface(QWidget):
    ...
    def init_bottom_panel(self):
        ...
        self.vBoxLayout.addLayout(
            bottom_layout, 2
        )  # Give more vertical space to bottom
                            ...
                            try:
                                ...
                                if dt >= cutoff_date:
                                    ...
                                    if len(row) > bait_cost_idx:
                                        self.history_daily_cost[date_str] += int(
                                            row[bait_cost_idx]
                                        )
                            except ValueError:
                                pass
        ...
        if records_path.exists():
            try:
                with open(records_path, "r", encoding="utf-8-sig") as f:
                    ...
                    header = next(
                        reader, None
                    )  # Timestamp,Name,Quality,Weight,IsNewRecord,Bait,BaitCost
    ...
    def _update_line_chart(self):
        ...
        series_sales.hovered.connect(
            lambda point, state: self._on_line_hover(point, state, "总收益")
        )
        ...
        series_cost.hovered.connect(
            lambda point, state: self._on_line_hover(point, state, "消耗成本")
        )
        pen = series_cost.pen()
        pen.setWidth(3)
        ...
        series_net.hovered.connect(
            lambda point, state: self._on_line_hover(point, state, "净收益")
        )
        pen = series_net.pen()
        pen.setWidth(3)
    ...
    def _get_nice_max_axis(self, value):
        ...
        if norm <= 1.0:
            step = (
                0.2 * order
            )  # shouldn't happen with floor log10 logic usually unless equal
        elif norm <= 2.0:
            step = 0.5 * order
        elif norm <= 5.0:
            step = 1.0 * order
    ...
    def _on_line_hover(self, point, state, name):
        ...
        if state:
            ...
            if (
                not hasattr(self, "line_chart_dates")
                or idx < 0
                or idx >= len(self.line_chart_dates)
            ):
                return
