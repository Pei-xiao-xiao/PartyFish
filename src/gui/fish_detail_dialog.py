...
from qfluentwidgets import (SubtitleLabel, BodyLabel, StrongBodyLabel, 
                            PushButton, CardWidget, qconfig, LineEdit)
...
class FishDetailDialog(QDialog):
    ...
    def _init_ui(self):
        ...
        if locations:
            ...
            if location_name:
                loc_badge = self._create_badge(
                    cfg._get_base_path() / "resources" / "location" / f"{location_name}.png",
                    location_name, "#2D5016", "#D4E7C5"
                )
        ...
        if locations:
            ...
            if conditions:
                ...
                if time_of_day:
                    cond_layout.addWidget(self._create_cell("时间", ", ".join(time_of_day)), 1)
                    cond_layout.addWidget(self._separator())
        ...
        quality_label.setStyleSheet("color: #8B7355; font-size: 14px; background: transparent; border: none;")
        ...
        weight_label.setStyleSheet("color: #8B7355; font-size: 14px; background: transparent; border: none;")
        ...
        kg_label.setStyleSheet("color: #A0826D; font-size: 14px; background: transparent; border: none;")
        ...
        if description:
            ...
            desc_label.setStyleSheet("color: #A0826D; font-size: 13px; line-height: 1.6; background: transparent; border: none;")
    ...
    def _create_badge(self, icon_path, text: str, text_color: str, bg_color: str) -> QFrame:
        """创建图标+文字徽章 - 优化设计"""
        badge = QFrame()
        ...
        label = QLabel(text)
        label.setStyleSheet(f"color: {text_color}; font-size: 13px; font-weight: 600; background: transparent; border: none;")
    ...
    def _create_cell(self, title: str, value: str) -> QWidget:
        ...
        t = BodyLabel(title)
        t.setStyleSheet("color: #A0826D; font-size: 12px; background: transparent; border: none;")
        t.setAlignment(Qt.AlignCenter)
        layout.addWidget(t)
        
        v = StrongBodyLabel(value)
        v.setStyleSheet("color: #5D4E37; font-size: 14px; font-weight: bold; background: transparent; border: none;")
