from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QLabel, 
                                QScrollArea, QFrame, QSizePolicy, QRadioButton, QButtonGroup)
from PySide6.QtCore import Qt, Signal, Property
from PySide6.QtGui import QColor, QFont
from qfluentwidgets import (StrongBodyLabel, BodyLabel, PushButton, 
                            FlowLayout, Theme, isDarkTheme, FluentIcon, themeColor)
...
class FilterPanel(QWidget):
    ...
    def _init_ui(self):
        ...
        # 活跃时间
        self._add_group("active_time", "活跃时间", 
                       ["凌晨", "清晨", "上午", "下午", "黄昏", "深夜"], 
                       content_layout)
        
        # 活跃天气
        self._add_group("active_weather", "活跃天气", 
                       options["weather"], 
                       content_layout)
        
        # 出没地点
        self._add_group("location", "出没地点", 
                       options["location"], 
                       content_layout)
        
        # 所需鱼竿
        self._add_group("rod_type", "所需鱼竿", 
                       pokedex.get_fish_types(), 
                       content_layout)
        
        # 收集状态
        self._add_group("collection_status", "收集状态", 
                       ["只显示未全部解锁", "只显示未解锁"], 
                       content_layout)
        
        # 活跃季节
        self._add_group("active_season", "活跃季节", 
                       options["season"], 
                       content_layout)
