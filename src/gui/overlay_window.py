...
from PySide6.QtGui import QMouseEvent, QPixmap, QPainter, QPainterPath, QColor, QPen, QFont, QFontDatabase
import os
...
class OverlayWindow(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # 尝试加载自定义字体作为备选
        font_path = os.path.join("resources", "fonts", "ZCOOLQingKeHuangYou-Regular.ttf")
        ...
        if os.path.exists(icon_path):
            ...
            icon_pixmap = QPixmap(icon_path).scaled(render_size, render_size, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def update_limit(self, remaining: int, current_sales: int = 0):
        ...
        if current_sales == 899:
            ...
            self.limit_label.setStyleSheet("color: #4CAF50; letter-spacing: 0.5px;")  # 绿色表示完美
            # 中文使用与状态相同的幼圆字体，12pt
            ...
            self.limit_label.setStyleSheet("color: #f57f17; letter-spacing: 0.5px;")  # 橙色警告
            # 中文使用与状态相同的幼圆字体，12pt
            limit_font = QFont(self._cute_font_family)
            limit_font.setPointSize(12)
            limit_font.setBold(True)
            ...
            if remaining < 100:
                self.limit_label.setStyleSheet("color: #f57f17; letter-spacing: 0.5px;")  # 橙色提醒
            else:
                self.limit_label.setStyleSheet("color: #8C6A51; letter-spacing: 0.5px;")  # 正常颜色
    ...
    def update_fish_preview(self):
        ...
        try:
            ...
            sorted_fish = pokedex.sort_fish(catchable_fish, sort_key='progress', reverse=False)
            ...
            for fish in sorted_fish[:5]:
                ...
                preview_item = FishPreviewItem(fish_name, str(image_path) if image_path else None)
