...
class HomeInterface(QWidget):
    ...
    def _connect_signals(self):
        ...
        self.banner_widget.screenshot_mode_changed.connect(
            self._on_screenshot_mode_changed
        )

        self.dashboard_widget.data_directory_requested.connect(
            self._open_data_directory
        )
        self.dashboard_widget.screenshot_directory_requested.connect(
    ...
    def _on_sound_toggled(self, checked: bool):
        """处理音效开关"""
        cfg.set_global_setting("control_sound_enabled", checked)
        cfg.save()
    ...
    def _on_release_mode_changed(self, mode: str):
        ...
        cfg.update_global_settings(
            {
                "release_mode": mode,
                "auto_release_enabled": mode == "auto",
            }
        )
        cfg.save()
        ...
        self.log_widget.append_log(
            f"[系统] 放生模式已切换为: {mode_text_map.get(mode, mode)}"
        )

    def _on_screenshot_mode_changed(self, mode: str):
        """处理截图模式变化"""
        cfg.set_global_setting("screenshot_mode", mode)
        cfg.save()
        mode_text_map = {"wegame": "WeGame", "steam": "Steam"}
        self.log_widget.append_log(
            f"[系统] 截图模式已切换为: {mode_text_map.get(mode, mode)}"
        )
    ...
    def _on_pokedex_data_changed(self):
        ...
        self.dashboard_widget.update_pokedex_progress(
            collected, total, collected_q, total_q
        )
        self.fish_preview_widget.refresh()
