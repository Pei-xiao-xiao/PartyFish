...
from PySide6.QtWidgets import (QWidget, QVBoxLayout, QHBoxLayout, QGridLayout, 
                             QTableWidgetItem, QHeaderView, QSplitter, QLabel, QGraphicsDropShadowEffect, QFrame)
from PySide6.QtGui import QColor, QBrush, QIcon, QDesktopServices, QPixmap, QPainter, QPainterPath, QFont, QLinearGradient
from qfluentwidgets import (CardWidget, TextEdit, StrongBodyLabel, TableWidget,
                            ...
                            HyperlinkButton, BodyLabel, ToolButton, IconWidget, PrimaryPushButton, ProgressRing, SegmentedWidget)
...
class HomeInterface(QWidget):
    ...
    def init_footer(self):
        ...
        self.author_label = BodyLabel(f"Created by R1ckyyyyy | {__version__} | 本软件完全开源免费", self)
        self.author_label.setTextColor(QColor(100, 100, 100), QColor(150, 150, 150))
        
        # Line 2: Disclaimer
        self.disclaimer_label = CaptionLabel("软件仅供学习交流，若因使用此软件导致封号与作者无关，请勿传播至公开平台", self)
    ...
    def init_banner(self):
        ...
        self.control_grid.addWidget(self.account_label, 0, 0, Qt.AlignRight | Qt.AlignVCenter)
        ...
        self.control_grid.addWidget(self.preset_label, 1, 0, Qt.AlignRight | Qt.AlignVCenter)
        ...
        self.overlay_switch.checkedChanged.connect(lambda _: self.toggle_overlay_signal.emit())
        self.control_grid.addWidget(self.overlay_label, 2, 0, Qt.AlignRight | Qt.AlignVCenter)
    ...
    def _create_styled_combobox(self, items: list, current: str, width: int) -> ComboBox:
        """创建样式化的下拉框"""
    ...
    def _create_rounded_pixmap(self, source: QPixmap, size: int, radius: int) -> QPixmap:
        """创建带圆角的高清头像"""
        ...
        # 绘制圆角矩形路径
        path = QPainterPath()
        path.addRoundedRect(0, 0, scaled_size, scaled_size, scaled_radius, scaled_radius)
    ...
    def init_dashboard(self):
        ...
        self.fish_collected_label.setStyleSheet("color: #0ea5e9; font-size: 15px; font-weight: bold;")
        ...
        self.crown_collected_label.setStyleSheet("color: #8b5cf6; font-size: 15px; font-weight: bold;")
    ...
    def _refresh_fish_preview(self):
        ...
        try:
            ...
            catchable_fish = pokedex.filter_fish_multi(all_fish, {"time": [current_time]})
            
            # 应用类型过滤
            filter_mode = getattr(cfg, 'fish_filter_mode', 'all')
            if filter_mode == 'lure':
                catchable_fish = [f for f in catchable_fish if '路亚' in f.get('type', '')]
            elif filter_mode == 'ice':
                catchable_fish = [f for f in catchable_fish if '冰钓' in f.get('type', '')]
            
            if not catchable_fish:
                # 显示空状态提示
                empty_label = CaptionLabel("当前时段暂无可钓鱼种", self.fish_cards_container)
                ...
                for fish in catchable_fish:
                    ...
                    is_fully_collected = all(status.get(q) is not None for q in QUALITIES)
                ...
                if not visible_fish:
                    # 所有可钓的都收集满了！
                    empty_label = CaptionLabel("当前时段所有鱼种已毕业！", self.fish_cards_container)
                    empty_label.setStyleSheet("color: #10b981; font-weight: bold; padding: 20px;")
                    self.fish_cards_layout.addWidget(empty_label)
                else:
                    # 按收集进度排序（未收集优先）
                    sorted_fish = pokedex.sort_fish(visible_fish, sort_key='progress', reverse=False)
    ...
    def update_sales_progress(self, sold: int, limit: int = 899):
        ...
        if is_overflow:
            ...
            self.sales_value_label.setText(f"{limit} / {limit} (<span style='color: #ef4444; font-weight: bold;'>+{overflow_amount} 已超额</span>)")
        else:
        ...
        def do_update_bar():
            ...
            self.sales_progress_fill.setStyleSheet(f"""
                QWidget {{
