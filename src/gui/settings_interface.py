...
class SettingsInterface(ScrollArea):
    ...
    def __init__(self, parent=None):
        ...
        self.sellHotkeyCard = SettingCard(FluentIcon.SHOPPING_CART, self.tr("卖鱼快捷键"), self.tr("设置用于一键卖鱼的快捷键。"))
        self.sellHotkeyLineEdit = KeyBindingWidget(self.sellHotkeyCard)
        self.sellHotkeyLineEdit.setFixedWidth(120)
        self.sellHotkeyCard.hBoxLayout.addWidget(self.sellHotkeyLineEdit, 0, Qt.AlignRight)
        margins = self.sellHotkeyCard.hBoxLayout.contentsMargins()
        self.sellHotkeyCard.hBoxLayout.setContentsMargins(margins.left(), margins.top(), 16, margins.bottom())
        ...
        self.autoClickSellCard = SwitchSettingCard(FluentIcon.ROBOT, self.tr("自动卖鱼点击"), self.tr("使用快捷键卖鱼时，识别后是否自动点击卖出按钮"))
        self.globalGroup.addSettingCard(self.autoClickSellCard)

        self.antiAfkCard = SwitchSettingCard(FluentIcon.VIEW, self.tr("视觉防挂机"), self.tr("检测挂机弹窗并自动点击恢复"))
        self.globalGroup.addSettingCard(self.antiAfkCard)

        self.soundAlertCard = SwitchSettingCard(FluentIcon.MUSIC, self.tr("提示音效"), self.tr("没鱼饵或鱼桶满时播放提示音"))
        ...
        self.createAccountCard = SettingCard(
            FluentIcon.ADD, self.tr("创建新账号"), 
            self.tr("输入新账号名称并点击创建"), parent=self.accountGroup)
        ...
        self.createAccountCard.hBoxLayout.addWidget(self.newAccountLineEdit, 0, Qt.AlignRight)
        self.createAccountCard.hBoxLayout.addSpacing(10)
        self.createAccountCard.hBoxLayout.addWidget(self.createAccountButton, 0, Qt.AlignRight)
        margins = self.createAccountCard.hBoxLayout.contentsMargins()
        self.createAccountCard.hBoxLayout.setContentsMargins(margins.left(), margins.top(), 16, margins.bottom())
        self.accountGroup.addSettingCard(self.createAccountCard)
        
        # Delete Account Card
        self.deleteAccountCard = SettingCard(
            FluentIcon.DELETE, self.tr("删除账号"), 
            self.tr("选择要删除的账号（不能删除当前账号）"), parent=self.accountGroup)
        ...
        self.deleteAccountCard.hBoxLayout.addWidget(self.deleteAccountComboBox, 0, Qt.AlignRight)
        self.deleteAccountCard.hBoxLayout.addSpacing(10)
        self.deleteAccountCard.hBoxLayout.addWidget(self.deleteAccountButton, 0, Qt.AlignRight)
        margins = self.deleteAccountCard.hBoxLayout.contentsMargins()
        self.deleteAccountCard.hBoxLayout.setContentsMargins(margins.left(), margins.top(), 16, margins.bottom())
    ...
    def _load_settings_to_ui(self, preset_name_to_load=None):
        ...
        self.soundAlertCard.setChecked(cfg.global_settings.get("enable_sound_alert", False))
    ...
    def _on_delete_account(self):
        ...
        w = MessageBox(
            self.tr("确认删除"),
            self.tr(f"确定要删除账号 '{account_name}' 吗？\n\n⚠️ 该账号的所有钓鱼记录和销售数据将被永久删除！"),
            self.window()
        )
