...
class FishingWorker(QThread):
                  ...
                  if found_region:
                      ...
                      for _ in range(10): # 尝试 10 次（约 2 秒）
                          ...
                          if initial_bait_before_cast is not None:
                              self.log_updated.emit(f"[调试] 抛竿前鱼饵数量: {initial_bait_before_cast}")
                              break
                          self.msleep(200)
                      ...
                      if initial_bait_before_cast is None:
                          ...
                          if cfg.global_settings.get("enable_sound_alert", False):
                              self.sound_alert_requested.emit("no_bait") # 复用 no_bait 音效
                      ...
                      # 如果超时，输出详细诊断信息
                      self.log_updated.emit(f"[诊断] 抛竿验证超时。检测次数: {verification_check_count}")
                      self.log_updated.emit(f"[诊断] 最后状态 - 抛竿图标已消失: {last_cast_icon_gone}, 等待图标已出现: {last_wait_icon_appeared}")
                      ...
                      if initial_bait_before_cast is not None and not self.paused:
                          ...
                          if final_bait is not None and final_bait < initial_bait_before_cast:
                              self.log_updated.emit(f"鱼饵减少 ({initial_bait_before_cast} -> {final_bait}) 但未成功收杆，判定为鱼跑了。")
                              self._record_event("鱼跑了")
                              return False
                      
                      # 如果当前处于暂停或停止状态，直接返回
                      if not self.running or self.paused:
                          self.log_updated.emit("抛竿验证期间被手动暂停或停止，重置状态。")
                          return False
    ...
    def _wait_for_bite(self):
        ...
        if initial_bait is not None:
                    ...
                    for key in ['F1_grayscale', 'F2_grayscale']:
                        if self.vision.find_template(key, region=cast_rod_region, threshold=0.8) or \
                           self.vision.find_template(key, region=cast_rod_ice_region, threshold=0.8):
                            self.log_updated.emit("检测到抛竿图标，可能未抛竿成功或已超时。重置循环。")
                            return False
            ...
            if initial_bait is None:
                # 仍然失败，使用特殊策略：监控任意变化
                self.log_updated.emit("警告: 无法获取初始鱼饵数量，将监控任意鱼饵变化。")
            ...
            if current_bait is None:
                ...
                if current_bait != initial_bait:
                    ...
                    if confirm_bait is not None and confirm_bait != initial_bait:
                        self.log_updated.emit(f"检测到鱼饵数量变化 ({initial_bait} -> {confirm_bait}), 判定为咬钩。")
                        return True
                    else:
                        # 疑似过渡帧误判，忽略本次检测
                        self.log_updated.emit(f"[调试] 疑似过渡帧，忽略 ({initial_bait} -> {current_bait} -> {confirm_bait})")
            ...
            if total_checks % unhook_check_interval == 0:
                ...
                if check_bait is not None and check_bait < initial_bait:
                    ...
                    for key in ['F1_grayscale', 'F2_grayscale']:
                        if self.vision.find_template(key, region=cast_rod_region, threshold=0.8) or \
                           self.vision.find_template(key, region=cast_rod_ice_region, threshold=0.8):
                            self.log_updated.emit(f"鱼饵减少 ({initial_bait} -> {check_bait}) 且抛竿图标出现，错过咬钩，鱼跑了！")
                            self._record_event("鱼跑了")
                            return False
        ...
        # 超时后输出详细统计
        self.log_updated.emit(f"等待咬钩超时。最后已知鱼饵: {last_known_bait}, 初始: {initial_bait}")
    ...
    def _record_catch(self):
        ...
        for ocr_attempt in range(max_ocr_retries):
            ...
            if result:
                ...
                if ocr_attempt < max_ocr_retries - 1:
                    ...
                    self.log_updated.emit(f"OCR 未识别到内容，等待 0.5 秒后重试 ({ocr_attempt + 1}/{max_ocr_retries})...")
        ...
        new_record_keywords = ["新纪录", "新记录", "新紀錄", "首次捕获", "首次捕獲", "首次"]
        ...
        try:
            ...
            if hasattr(cfg, 'fish_names_list') and cfg.fish_names_list:
                ...
                matches = difflib.get_close_matches(search_name, cfg.fish_names_list, n=1, cutoff=0.6)
                
                # 如果 search_name 匹配失败且与 fish_name 不同，再用原始名尝试一次
                if not matches and search_name != fish_name:
                    matches = difflib.get_close_matches(fish_name, cfg.fish_names_list, n=1, cutoff=0.6)
                
                if matches:
                    if fish_name != matches[0]:
                        self.log_updated.emit(f"鱼名校正: '{fish_name}' -> '{matches[0]}'")
                    fish_name = matches[0]
                else:
                    # 匹配失败时的最后清理逻辑：移除尾部噪声
                    if re.search(r'[\u4e00-\u9fa5]', fish_name):
        ...
        try:
            ...
            with open(csv_file, 'a', encoding='utf-8') as f:
                ...
                f.write(f'{timestamp},{fish_name},{quality},{weight},{is_new_record_str},{bait_name},{bait_cost}\n')
    ...
    def _record_event(self, event_type: str):
        ...
        try:
            ...
            event_data = {'name': event_type, 'weight': '', 'quality': '', 'is_new_record': False}
    ...
    def trigger_sell(self):
        ...
        try:
            ...
            # 使用鱼干图标作为锚点定位数字区域
            fish_icon_result = self.vision.find_template_in_image('fish_icon_grayscale', image, threshold=0.5)
            ...
            if fish_icon_result:
                ...
                self.log_updated.emit(f"[调试] 鱼干图标: 中心({center_x},{center_y}), 尺寸({icon_width}x{icon_height}), 数字起始: {icon_right_x}")
            else:
                # 未找到鱼干图标，使用整个区域（兜底方案）
            ...
            if self._write_sale_record(amount):
                ...
                self.log_updated.emit("严重错误：销售记录保存失败！为防止数据丢失，已终止出售操作。")
            ...
            try:
                ...
                if cfg.screen_width <= 0 or cfg.screen_height <= 0:
                    if not minimized_log_printed:
                        self.log_updated.emit("检测到游戏窗口最小化，已暂停检测，等待窗口恢复...")
                ...
                if self.vision.find_template_popup('exclamation_grayscale', region=popup_region, threshold=0.7):
                    ...
                    # 加上窗口偏移转换为屏幕绝对坐标
                    self.inputs.click(target_x + cfg.window_offset_x, target_y + cfg.window_offset_y)
