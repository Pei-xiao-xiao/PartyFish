...
class Pokedex(QObject):
    ...
    def get_filter_options(self) -> Dict[str, List[str]]:
        ...
        sorted_season = sorted(
            list(options["season"]),
            key=lambda x: season_order.index(x) if x in season_order else 99,
        )
    ...
    def filter_fish_multi(
        self, fish_list: List[Dict], criteria: Dict[str, List[str]]
    ) -> List[Dict]:
        """
        多条件组合筛选
        criteria 结构: {
    ...
    def sort_fish(
        self, fish_list: List[Dict], sort_key: str, reverse: bool = False
    ) -> List[Dict]:
        """
        对鱼类列表进行排序
        ...
        if sort_key == "name":
            ...
            weight_map = {"传奇": 16, "史诗": 8, "稀有": 4, "非凡": 2, "标准": 1}
            ...
            return sorted(
                fish_list, key=lambda f: -get_uncollected_score(f), reverse=reverse
            )
