...
- 钓鱼记录：`%APPDATA%\Partyfish\accounts\默认账号\data\records.csv`
- 销售记录：`%APPDATA%\Partyfish\accounts\默认账号\data\sales.csv`
...
├── %APPDATA%/Partyfish/             # 用户数据目录（运行时生成）
│   ├── config.json                  # 用户配置
│   └── accounts/默认账号/data/      # 默认账号运行数据
